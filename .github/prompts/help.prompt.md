...
| **Google API** | OAuth errors, credential issues | Check `~/.todo-app/credentials.json` exists, verify API is enabled |

### 3. If the UI or Instructions Don't Match Reality

Google Cloud Console, VS Code, and GitHub Copilot update frequently. If the user reports that a screen looks different from what's described in README.md or the setup guide:
